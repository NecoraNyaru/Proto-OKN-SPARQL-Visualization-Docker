...
RUN mkdir -p $JENA_FUSEKI_HOME/rdf-data \
    && LATEST_RELEASE_URL=$(curl -s https://api.github.com/repos/NecoraNyaru/Proto-OKN-SPARQL-Visualization-Docker/releases/latest | jq -r '.assets[] | select(.name=="secure-chain.ttl") | .browser_download_url') \
